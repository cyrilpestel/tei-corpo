...
public class HT_ToTei{
	...
	public void buildEmptyTEI(){
		...
		//revisionDesc.setAttribute("url", ht.filePath.split("\\.")[0]  + Utils.EXT);
	...
	public void addTemplateDesc(){
		...
		for(int i = 0; i<persons.getLength(); i++){
			...
			if(person.getElementsByTagName("alt").getLength()>0){
				...
				String parent = ht.tiersInfo.get(alt.getAttribute("type")).parent;
				...
				String type = ht.tiersInfo.get(alt.getAttribute("type")).type.constraint;
				if(Utils.isNotEmptyOrNull(type)){
		...
		*/
		for(Map.Entry<String , TierInfo> entry : ht.tiersInfo.entrySet()){
			...
			if(Utils.isNotEmptyOrNull(tierInfo.type.cv_ref)){
				// System.out.println("Ajout de " + tierInfo.type.cv_ref + " dans " + entry.getKey());
...
//			System.out.println("Value is: "+me.getValue());
		...
		/*
		Double whenVal = (double) -1;
		ArrayList<Element> timesList = new ArrayList<Element>();
		for(Entry<String, String> entry : ht.timeline.entrySet()){
			Element when = docTEI.createElement("when");
			when.setAttribute("interval", entry.getValue());
			when.setAttribute("xml:id", entry.getKey());
			when.setAttribute("since", "#T0");
			timesList.add(when);
			if(Utils.isNotEmptyOrNull(entry.getValue()) && Double.parseDouble(entry.getValue())>whenVal){
				whenVal = Double.parseDouble(entry.getValue());
			}
		}
		if(whenVal!=-1){
			//Ajout durée doc
			duration = Double.toString(whenVal);
			Element recording = (Element)docTEI.getElementsByTagName("recording").item(0);
			recording.setAttribute("dur", duration);
		}
		Utils.sortTimeline(timesList);
		for(Element when : timesList){
			timeline.appendChild(when);
		}
		*/
	}
...
//		Element lastAU = (Element) mainDiv.getElementsByTagName(Utils.ANNOTATIONBLOC).item(mainDiv.getElementsByTagName(Utils.ANNOTATIONBLOC).getLength()-1);
	...
	public void buildDependances(Element annotUEl, ArrayList<Annot> depAnnots){
		...
		for(int i = 0; i<depAnnots.size(); i++){
			...
			if(!dependAnnotIds.contains(annot.id)){
				...
				//System.out.println(annot.content.trim() + " --- >>> " + annot.dependantAnnotations );
				if(Utils.isNotEmptyOrNull(annot.id)){
