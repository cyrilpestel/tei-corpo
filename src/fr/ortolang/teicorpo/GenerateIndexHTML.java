...
public class GenerateIndexHTML {
	...
	public void writeIndexHTML() throws IOException{
		...
		out.println( "<link rel=\"stylesheet\" type=\"text/css\" href=\"http://ct3.ortolang.fr/stylesheets/indexStyle.css\"/>" ); 

		out.println( "<link href=\"<?php print($BOOTSTRAP_HOST) ?>http://ct3.ortolang.fr/stylesheets/bootstrap.min.css\" rel=\"stylesheet\">" ); 
		out.println( "<link href=\"<?php print($BOOTSTRAP_HOST) ?>http://ct3.ortolang.fr/stylesheets/bootstrap-responsive.min.css\" rel=\"stylesheet\">" ); 
		out.println( "<link href=\"<?php print($SITE_HOST) ?>http://ct3.ortolang.fr/stylesheets/docs.css\" rel=\"stylesheet\">" ); 
		out.println( "<link href=\"<?php print($SITE_HOST) ?>http://ct3.ortolang.fr/stylesheets/reflow-skin.css\" rel=\"stylesheet\">" ); 
		out.println( "<link href=\"<?php print($HIGHLIGHTJS_HOST) ?>http://ct3.ortolang.fr/styles/github.min.css\" rel=\"stylesheet\">" ); 
		out.println( "<link href=\"<?php print($SITE_HOST) ?>http://ct3.ortolang.fr/stylesheets/site.css\" rel=\"stylesheet\">" ); 
		out.println( "<link href=\"<?php print($SITE_HOST) ?>http://ct3.ortolang.fr/stylesheets/print.css\" rel=\"stylesheet\" media=\"print\">" ); 
		out.println();
		out.println( "<link rel=\"stylesheet\" href=\"http://ct3.ortolang.fr/stylesheets/dialog-style.css\">" ); 
		out.println( "<link rel=\"stylesheet\" href=\"http://ct3.ortolang.fr/stylesheets/font-awesome-4.3.0/css/font-awesome.min.css\">" ); 
		...
		out.println( "print(\"<link rel=\\\"stylesheet\\\" type=\\\"text/css\\\" href=\\\"http://ct3.ortolang.fr/$CSS\\\"></link>\");" ); 
		out.println( "}"); 
		out.println( "}");
		
		out.println( "if(isset($HEADER_JS)) {" ); 
		out.println( "foreach($HEADER_JS as $JS) {" ); 
		out.println( "print(\"<script language=\\\"javascript\\\" type=\\\"text/javascript\\\" src=\\\"http://ct3.ortolang.fr/$JS\\\"></script>\");" ); 
		out.println( "}		" ); 
		out.println( "}" ); 
		...
		out.println( "<script type=\"text/javascript\" async=\"\" src=\"http://www.google-analytics.com/ga.js\"></script><script type=\"text/javascript\">" ); 
		...
		out.println( "<span class=\"icon-bar\"></span>");
		out.println( "</a>");
		out.println( "<a class=\"brand\" href=\"<?php print($SITE_HOST) ?>http://ct3.ortolang.fr/index.php?page=accueil\"><img src=\"<?php print($SITE_HOST) ?>http://ct3.ortolang.fr/images/ortolang-logo.png\" style=\"height:40px; margin-right:10px;\"><span class=\"color-highlight\">ORTOLANG</span> Nanterre/Orléans</a>");
		out.println( "</div>");
		out.println( "</div>");
		out.println( "</div>");
		...
		File [] listFilesMedias = inputDir.listFiles(new FilenameFilter(){
			public boolean accept(File dir, String name) {
				return name.endsWith(".mov") || name.endsWith(".mp4") || name.endsWith(".webm") || name.endsWith(".ogv") || name.endsWith("ogg")  || name.endsWith(".mp3") || name.endsWith(".wav");
			}
		});
		...
		if(listFilesHTML.length > 0){
			...
			for (File htmlFile : listFilesHTML){
				...
				if (htmlFileName.endsWith(".tei_corpo.xml")) {
					...
					out.printf("<li>Visualisation sous-titres : <a href=\"%s\">%s</a></li>%n", htmlFileName, htmlFileName);
				}

			/*
			out.printf("<li>Visualisation classique : <button id=\"bclassic\">%s</button></li>%n", baseName);
			out.printf("<li>Visualisation sous-titres : <button id=\"bsubt\">%s</button></li>%n", htmlSubtFileName);
			out.printf("<li>Visualisation texte : <button id=\"btrjs\">%s</button></li>%n", baseName);

			out.println("</ul>");
			
			out.println( "<dialog id=\"trjs\">");
			out.println( "<div>");
			out.printf( "<iframe onload=\"this.width=window.innerWidth*0.95;this.height=window.innerHeight*0.95;\" id=\"idtrjs\" src=\"%s\"></iframe>%n", textVisuAdress);
			out.println( "<button id=\"oktrjs\">");
			out.println( "<i class=\"fa fa-times\"></i>");
			out.println( "</button>");
			out.println( "</div>");
			out.println( "</dialog>");

			out.println( "<dialog id=\"classic\">");
			out.println( "<div>");
			out.println( "<!--");
			out.println( "<div style='position: relative; width: 100%; height: 0px; padding-bottom: 60%;'>");
			String style = "position: absolute; left: 0px; top: 0px; width: 100%; height: 100%;";
			out.printf( "<iframe style='%s' id=\"idclassic\" src=\"%s\"></iframe>%n", style, htmlFileName);
			out.println( "</div>");
			out.println( "-->");
			out.printf( "<iframe onload=\"this.width=window.innerWidth*0.9;this.height=window.innerHeight*0.9;\" id=\"idclassic\" src=\"%s\"></iframe>%n", htmlFileName);
			out.println( "<button id=\"okclassic\">");
			out.println( "<i class=\"fa fa-times\"></i>");
			...
			out.println( "<dialog id=\"vid\">");
			out.println( "<div>");
			out.println( "<video id=\"idvid\" controls>");
			out.printf( "<source src=\"%s-240p.mp4\">%n", baseName);
			out.println( "Votre navigateur ne gère pas l'élément <code>video</code>.");
			out.println( "</video>");
			out.println( "<button id=\"okvid\">");
			out.println( "<i class=\"fa fa-times\"></i>");
			out.println( "</button>");
			out.println( "</div>");
			out.println( "</dialog>");

			out.println( "<dialog id=\"subt\">");
			out.println( "<div>");
			out.println( "<!--");
			out.println( "-->");
			out.printf( "<iframe onload=\"this.width=window.innerWidth*0.8;this.height=window.innerHeight*0.6;\" id=\"idsubt\" src=\"%s\"></iframe>%n", htmlSubtFileName);
			out.println( "<button id=\"oksubt\">");
			out.println( "<i class=\"fa fa-times\"></i>");
			out.println( "</button>");
			out.println( "</div>");
			out.println( "</dialog>");
		...
		if(listFilesMedias.length > 0){
			...
			for (File mediaFile:listFilesMedias){
				...
				out.printf("<a href=\"%s\">%s</a> <?php echo getSize(\"%s\"); ?><br/></li>%n", fileName, fileName, fileName);
			}
			out.println("</ul>");
		}
		...
		out.println("<a href=\"http://portail.ortolang.fr/index.php?page=presentation\" title=\"Présentation\">Présentation </a>");
		out.println("</li>");
		out.println("<li <?php if($submenu==\"partenaires\") { print(\"class=\\\"active\\\"\"); }?>>");
		out.println("<a href=\"http://portail.ortolang.fr/index.php?page=partenaires\" title=\"Partenaires\">Partenaires </a>");
		out.println("</li>");
		...
		out.println("<a href=\"http://portail.ortolang.fr/index.php?page=mentionslegales\" title=\"Mentions Légales\">Mentions Légales </a>");
		out.println("</li>");
		out.println("</ul>");
		out.println("</div>");
		out.println("<div class=\"span2 bottom-nav\">");
		...
		out.println("<a href=\"http://portail.ortolang.fr/search/index.php?search=&rightsOfUse=http%3A%2F%2Fwww.ortolang.fr%2Fontology%2Ffree&rightsOfUse_operator=equal&lang=fr\" title=\"Recherche globale\">Recherche globale </a>");
		out.println("</li>");
		out.println("<li>");
		...
		out.println("<li>");
		out.println("<a href=\"http://portail.ortolang.fr/index.php?page=documentation\" title=\"Documentation Utilisateur\">Documentation Utilisateur</a>");
		out.println("</li>");
		out.println("<li>");
		...
		out.println("<p class=\"copyright\">Copyright ©2014 <a href=\"http://dev.ortolang.fr/team-list.html\">&Eacute;quipe Ortolang</a>. All Rights Reserved.</p>");
		out.println("</div>");
		out.println("</div>");
		out.println("</div>");
		...
		out.println("http://stackoverflow.com/questions/1014203/best-way-to-use-googles-hosted-jquery-but-fall-back-to-my-hosted-library-on-go -->");
		...
		out.println("{");
		out.println("document.write(unescape(\"%3Cscript src='./scripts/js/jquery-1.8.3.min.js' type='text/javascript'%3E%3C/script%3E\"));");
		out.println("}");
		out.println("</script>");
		...
		out.println("<script src=\"<?php print(\"{$HIGHLIGHTJS_HOST}{$HIGHLIGHTJS_JS_HOST}\") ?>highlight.min.js\"></script>");
		...
		out.println("print(\"<script language=\\\"javascript\\\" type=\\\"text/javascript\\\" src=\\\"$JS\\\"></script>\");");
		out.println("}");
		out.println("}");
