	...
	public class AnnotatedUtterance {
		...
		public AnnotatedUtterance(Element annotatedU) {
			...
			if (optionsOutput != null) {
				if (optionsOutput.isDontDisplay(speakerCode))
					return;
				if (!optionsOutput.isDoDisplay(speakerCode))
					return;
			}
					...
					// Ajout des tiers
						...
						if (optionsOutput != null) {
							if (optionsOutput.level == 1)
								return;
							if (optionsOutput.isDontDisplay(type))
								return;
							if (!optionsOutput.isDoDisplay(type))
								return;
						}
