...
public class ElanToTei {
	...
	public static void usage() {
		System.err.println("Description: ElanToTei convertit un fichier au format Elan en un fichier au format TEI");
	...
	public static void submain(String[] args) throws Exception {
		...
		if (f.isDirectory()) {
			// Parcours de tous les fichiers présents dans le repertoire, et
			// descente dans les sous-repertoires...
			...
			for (File file : elanFiles) {
				...
				if (file.isFile()) {
					if (name.endsWith(Utils.EXT_PUBLISH + EXT)) {
						...
						// file.toString(), outputDir + outputFileName);
